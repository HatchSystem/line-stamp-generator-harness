...
`AskUserQuestion` は使用しない。別の同期・非同期の質問 UI ツールにも置き換えない。共通対話ルールの作業の区切り条件を満たしてから、メインエージェントが通常のチャットの最終応答で質問してターンを終了する。回答を受けるまで次の作業を始めない。具体的な提示方法と承認の記録は `AGENTS.md` と共通スキルの対話手順に従う。
