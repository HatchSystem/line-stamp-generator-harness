...
人物写真由来・既存キャラ由来のどちらでも、タイトルと説明へ実名を書かず承認されたキャラ名を使う。素材利用権が不明、または写真の本人許諾が `yes` でない場合は制作自体を止める。権利と許諾を満たしていても写真の成年確認が `no|unknown`、またはユーザーが審査申請を希望しない場合は `publish: local-only` とし、公開申請を案内しない。未申請の成果物は LINE 上で配布・利用できない。

用意する案（`meta/submission.json` に保存。形式は [submission.example.json](../assets/submission.example.json)）:

- スキーマ: `schema_version: 2` を維持する。旧形式は先に公開 CLI の `project --root . migrate` で診断する
...
- ライセンス証明: `license_proof.status` は `not-required|user-confirmed`、`reference` はユーザーが提示した作品の HTTPS URL、または対象プロジェクトを基準にした既存の相対ファイルパス（例: `refs/license.pdf`）を記録する。`photo_used: true` または `rights: licensed` では `user-confirmed` と空でない参照が必須。エージェントは証明書を作らず、検査も法的有効性を保証しない
- LINEスタンプ プレミアム: 登録画面の既定を黙認せず、参加するかをユーザーへ確認し `premium_participation` の真偽値で保存する
- タグ案: 各スタンプに最大9個。P3の表情・用途から機械的に候補を作る

`python scripts/line_stamp.py check-publish-ready ...` を通し、エラー0でユーザーにメタ案を提示する。承認後に [publish.md](publish.md) の P8 へ進む。
