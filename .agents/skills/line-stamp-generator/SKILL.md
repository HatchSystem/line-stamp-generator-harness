...
1. P0: 入力種別（写真／既存キャラ）、素材、素材の著作権・商用利用権、写真なら被写体本人の許諾と成年確認、枚数（8/16/24/32/40）、文字、文字の入れ方（`font` フォント合成＝既定・生成モデルによる文字崩れを避ける／`ai` 生成AIに描かせる＝誤字検査あり）、キャラ名、01の候補数、LINE への審査申請意図を確定する。権利不明または写真の本人許諾なしなら制作を開始しない。
...
8. P7: `publish: yes` の場合だけ、実名を使わない申請メタ案、販売エリア、AI・写真使用、ライセンス証明、LINEスタンプ プレミアム参加設定を出し、`check-publish-ready` を通して承認を得る（[references/application.md](references/application.md)）。
...
- 制作には素材の著作権・商用利用権（`rights: own|licensed`）を必須とし、権利不明ならローカル成果物も作らない。写真案件は被写体本人の許諾（`consent: yes`）も常に必須にする。写真の `adult` が `no|unknown` の場合は、権利と許諾があっても `publish: local-only` のローカル確認用成果物（LINE 上で配布・利用不可）までとする。既存キャラクター案件の `consent` と `adult` は `n/a` にする。
