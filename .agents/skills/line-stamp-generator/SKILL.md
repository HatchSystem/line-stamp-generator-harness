...
1. P0: 入力種別（写真／既存キャラ）、素材、枚数（8/16/24/32/40）、文字、文字の入れ方（`font` フォント合成＝既定・生成モデルによる文字崩れを避ける／`ai` 生成AIに描かせる＝誤字検査あり）、キャラ名、01の候補数、LINE への審査申請意図を確定する。`ai` を確定する前にTesseractまたは独立画像認識の利用可否を診断し、両方なければ `font` への変更か準備待ちを選んでもらう。成年、本人許諾、著作権／ライセンスは質問しない。
...
6. P5: 承認済み三面図を毎回参照して残りを作り、全画像を1枚に載せた白系・濃色背景の版付き確認一覧を必ず提示して承認を得る。`ai` は全点の自動文字検査と版付き文字領域マスクを作り、エージェントが読み上げ、ユーザーが「全点正しい」と答えたら `text_check: ok` と `text_mask_version` を記録する。誤字は該当番号だけ再生成する。
