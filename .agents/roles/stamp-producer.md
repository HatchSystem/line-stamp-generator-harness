...
3. `text_mode: font` では生成モデルに文字を描かせず、決定論的に合成する。`text_mode: ai` では承認セリフだけを描かせ、自身が全画像を実際に開いて文字を目視確認し、読み取りと判定を版付きで記録し、文字領域マスクを作る。未確認・不一致・判読困難を合格にしない。結果をメインエージェントへ返し、P4採用／P5一覧承認を受ける。文字だけの追加承認や独立モデルの二重検査は不要
